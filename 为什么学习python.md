...
作为一名专业软件开发人员，您可能要处理 C/C++/Java 库，但总感觉编码、编译、测试、再编译等开发流程太慢了；也许您正在给这些库开发测试套件，但总觉得这项工作真是枯燥乏味。又或许，您开发了个使用扩展语言的软件，却不想为这个软件专门设计一种新语言。

那么，Python 正好能满足您的需要。虽然，Unix 脚本或 Windows 批处理文件也可以完成这些任务，但 shell 脚本擅长的是移动文件和替换文本，并不擅长 GUI 界面或游戏开发。当然，您也可以编写 C/C++/Java 程序，但开发第一个版本的初稿可能就要耗费很长时间。Python 则简单易用，既可以在 Windows、Mac OS X、Unix 等操作系统上使用，也可以让您更快完成任务。
